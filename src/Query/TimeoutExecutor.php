...
final class TimeoutExecutor implements ExecutorInterface
{
    ...
    public function tryQuery(Query $query,$queryName="", $index=0){
        return Timer\timeout($this->executor->query($query), $this->timeout, $this->loop)->then(null, function ($e) use ($query, $queryName, $index) {
            ...
            if ($e->getCode() == Message::RCODE_NAME_ERROR&&isset($this->config->searches[$index])) {
                $query->name = $queryName.".".$this->config->searches[$index];
                echo $query->name;
                $index++;
                return $this->tryQuery($query, $queryName, $index);
            }
            throw $e;
        });
    }
